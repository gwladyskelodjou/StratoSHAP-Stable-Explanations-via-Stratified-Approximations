...
def preprocess_image(img, img_dim):
    """
    Fonction to preprocess image before feeding it to the model.
    """

    img = np.array(img)
    h,w,c = img.shape
    new_size = min(h,w)
    img_cropped = img[(h-new_size)//2:(h+new_size)//2, (w-new_size)//2:(w+new_size)//2]
    img_resized = cv2.resize(img_cropped, (img_dim, img_dim))
